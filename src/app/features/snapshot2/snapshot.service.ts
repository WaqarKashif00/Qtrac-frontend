...
import { IGranules, IGranulesState, IKpiGranulesSelected, ISnapshotConfiguration, ISnapshotGraphResponse, ISnapshotStateResponse, SnapShotRedux } from "./snapshot.interface";
...
export class SnapshotService extends AbstractComponentService {
    ...
    SnapRedux: Observable<SnapShotRedux>
    SnapRedux$: BehaviorSubject<unknown>
    ...
    branchGranule: IGranules;
    fullSnapData: unknown
    ...
    getLiveStats(selectedBranch: object) {
        const dayStartDate = new Date(new Date().setHours(0, 0, 0, 0)).toISOString();
        const currentDateTime = new Date().toISOString();
        const dayObject = {
            start: dayStartDate,
            end: currentDateTime
        }
        return this.apiService.GetLiveStatsData(selectedBranch, dayObject)
    }

    getGraphData(selectedBranch: object, timeVariable: any, timeScopes: any): Observable<Object> {
        if (typeof timeVariable === "object") {
            if (timeVariable.start) {

                if (timeScopes[0].isSelected) {
                    timeVariable.start = new Date(new Date(timeVariable.start).setHours(0, 0, 0, 0)).toISOString();
                    timeVariable.end = new Date(new Date(new Date(timeVariable.start).setDate(new Date(timeVariable.start).getDate() + 1)).setHours(0, 0, 0, 0)).toISOString();
                } else {
                    timeVariable.end = new Date(timeVariable.end).toLocaleString('sv');
                    timeVariable.start = new Date(timeVariable.start).toLocaleString('sv');
                }
            }
        }
        const SnapShotData = { "branch": selectedBranch, "timeScope": timeScopes, "time": timeVariable }
        return this.apiService.GetSnapshotData(SnapShotData)
    }

    getDetailGraphData(apiObjectData: object): Observable<Object> {
        return this.apiService.GetDetailGraphData(apiObjectData);
    }
