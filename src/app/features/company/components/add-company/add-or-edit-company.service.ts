...
import { ICompanyConfigDropdownListData } from 'src/app/features/company-configuration/models/company-configuration-dropdownlist-data.interface';
import { PhoneNumber } from 'src/app/features/company-configuration/models/company-phone-number.interface';
import { ITime } from 'src/app/features/scheduler/hours-of-operations/hours-of-operation.interface';
import { IWorkflowDropDown } from 'src/app/features/work-flow/models/worflow-dropdown.interface';
...
import { ICompanyConfiguration } from '../../../company-configuration/models/company-configuration.interface';
import { ITwilioCredentials } from './models/company-configuration-twilio-creadentials.interface';
import { ICountryDropdownList } from './models/country-dropdown-list.interface';
...
@Injectable()
export class AddOrEditCompanyService extends AbstractComponentService {
  ...
  private SubjectCompanyConfigDropdownListData: Subject<ICompanyConfigDropdownListData>;
  ...
  public CompanyConfigDropdownListData: ICompanyConfigDropdownListData;
  public StateList: IStateDropdownList[];
  public SubjectIsAddMode: BehaviorSubject<boolean>;
  ...
  public SubjectIsValidSMTPSettings: BehaviorSubject<boolean>;
  public IsValidSMTPSettings$: Observable<boolean>;
  public CompanyId = this.browserStorageService.CompanyId;
  ...
  get CountryFormControl() {
    return this.CompanyConfigForm.get('companyConfigurationInfo.country');
  }
  ...
  public SetObservables() {
    ...
    this.SubjectCompanyConfigDropdownListData =
      new Subject<ICompanyConfigDropdownListData>();
    ...
    this.SubjectIsAddMode = new BehaviorSubject<boolean>(false);
    this.IsAddMode$ = this.SubjectIsAddMode.asObservable();
    ...
    this.SubjectIsValidSMTPSettings = new BehaviorSubject<boolean>(false);
    this.IsValidSMTPSettings$ = this.SubjectIsValidSMTPSettings.asObservable();
    ...
    this.CompanyConfigDropdownListData$.subscribe((x) => {
      this.CompanyConfigDropdownListData = x;
    });
  ...
  public SetCompanyFormGroup(companyDetailsData: ICompanyConfiguration) {
    this.CompanyConfigForm = this.formBuilder.group({
      companySuperAdminInfo: this.formBuilder.group({
        ...
        twilioServiceId: [companyDetailsData.companySuperAdminInfo.twilioServiceId],
        messangiBearerToken: [companyDetailsData.companySuperAdminInfo.messangiBearerToken],
        smsGatewayName: [companyDetailsData.companySuperAdminInfo.smsGatewayName],
        isAccountLinked: [companyDetailsData.companySuperAdminInfo.isAccountLinked],
        useLaviSMTPSettings: [true] // TODO : Disabled this section discussed in this task QTVR-1310
      }),
      companyConfigurationInfo: this.formBuilder.group({
        ...
        address: [companyDetailsData.companyConfigurationInfo.address],
        laviAddress: [
        ...
        city: [companyDetailsData.companyConfigurationInfo.city],
        state: [companyDetailsData.companyConfigurationInfo.state],
      ...
      companyCommunicationInfo: this.formBuilder.group({
        ...
        displayName: [companyDetailsData.companyCommunicationInfo.displayName],
        smsTotalNumber: [
  ...
  public GetDefaultCompanyDetails() {
    const defaultNulldata: ICompanyConfiguration = {
      ...
      companyConfigurationInfo: {
        ...
        phoneNumber: null,
        city: null,
        state: DefaultCompanyConfigValues.StateListDefaultValue,
        zip: '',
  ...
  public SetMode() {
    ...
    this.SubjectIsAddMode.next(this.IsAddMode);
  }
  ...
  public GetCompanyConfigurationDetails(): void {
    this.subs.sink = this.companyAPIService
      ...
      .subscribe((res: IRequest) => {
        if (res !== undefined && res !== null) {
          ...
          this.SubjectIsValidSMTPSettings.next(res.isValidSMTPSettings);
  ...
  BindSMSConfigurations() {
    if (this.IsAccountLinked()) {
      this.BindPhoneNumbers(this.CompanyId);
    }
  }
  ...
  public AddTagToSuggestionList(tags: string[]) {
    ...
    if (tags && this.CompanyConfigDropdownListData) {
      for (const tag of tags) {
        if (
          this.CompanyConfigDropdownListData.tagList.indexOf(tag) ===
          notFoundIndex
        ) {
          this.CompanyConfigDropdownListData.tagList.push(tag);
        }
      }
    }
  }
  ...
  public SetFormGroupValues(formData: ICompanyConfiguration) {
    ...
    this.CompanyConfigForm.get('companySuperAdminInfo.isAccountLinked').setValue(
      formData.companySuperAdminInfo.isAccountLinked
    );
    this.CompanyConfigForm.get('companySuperAdminInfo.useLaviSMTPSettings').setValue(
    ...
    this.CompanyConfigForm.get('companyConfigurationInfo.address').setValue(
      formData.companyConfigurationInfo.address
    );

    this.CompanyConfigForm.get('companyConfigurationInfo.laviAddress').setValue(
      formData.companyConfigurationInfo.laviAddress
    );
    ...
    this.SMTPServiceProviderControl.setValue(
      this.sharedCompanyConfigurationService.GetSMTPServiceProviderObject(
        formData.companyCommunicationInfo
      )
    );
    ...
    this.CompanyConfigForm.get('companyCommunicationInfo.displayName').setValue(
      formData.companyCommunicationInfo.displayName
    );
    ...
    this.CompanyConfigForm.get(
      'companyAdvanceSettingInfo.endOfDayTimeFormControl'
    ).setValue(formData.companyAdvanceSettingInfo.endOfDayTimeFormControl);
  ...
  public GetResponseData(res: IRequest): ICompanyConfiguration {
    const getResponseData: ICompanyConfiguration = {
      ...
      companyConfigurationInfo: {
        ...
        city: null,
        country: this.GetCountryObj(res.countryCode)
        ...
        isActive: this.SetDefaultIfUndefined(res.isActive),
  ...
  public GetCountryObj(countryCode: string): ICountryDropdownList {
    return this.CompanyConfigDropdownListData === undefined
      ? null
      : this.CompanyConfigDropdownListData.countryList.find(
  ...
  public GetTimeIntervalObj(id: string): IDropdown {
    return this.CompanyConfigDropdownListData === undefined
      ? null
      : this.CompanyConfigDropdownListData.timeIntervalList.find(
          (x) => x.value === id
        );
  }
  ...
  public GetLoginModeObj(id: string): IDropdown {
    return this.CompanyConfigDropdownListData === undefined
      ? null
      : this.CompanyConfigDropdownListData.loginModeList.find(
          (x) => x.value === id
        );
  }
  ...
  public GetEncryptionObj(id: string): IDropdown {
    return this.CompanyConfigDropdownListData === undefined
      ? null
      : this.CompanyConfigDropdownListData.encryptionList.find(
          (x) => x.value === id
        );
  }
    ...
    this.subs.sink = this.formService
      ...
      .subscribe(
        ([
          ...
          if (this.IsAddMode) {
            const language = DropdownData.languageList.filter(
              (language) => language.languageCode === Language.English
            );
            this.SupportedLanguageControl.setValue(language);
  ...
  public async GetBranch(branchId): Promise<any> {
  ...
  public GetTagList() {
    return this.companyAPIService.GetTags<string>();
  }
  ...
  public BindStateList(countryId: string, fn): void {
    ...
    this.CompanyConfigForm.get('companyConfigurationInfo.zip').setValue('');
    this.subs.sink = this.locationAPIService
      .GetStates(countryId)
      .subscribe((res) => {
        this.StateList = res;
  ...
  public SaveCompanyConfiguration(req: FormGroup) {
    if (
      this.authService.UserType === UserType.Lavi &&
  ...
  public GetRequest(result: ICompanyConfiguration): IRequest {
    ...
    const data: IRequest = {
      pk: this.IsAddMode ? id : this.CompanyId,
      type: this.TypeAsData,
      companyId: this.IsAddMode ? id : this.CompanyId,
      ...
      isDeleted: false, // result.companyConfigurationInfo.isDelete,
      isActive: true, // result.companyConfigurationInfo.isActive,
  ...
  public Cancel() {
    this.routeHandlerService.RedirectToCompany();
  }

  private async PostCompanyConfigurationDetails(
    ...
    data.superAdminConfig.isAccountLinked = validation;
    this.subs.sink = this.companyAPIService
      .Create<IRequest, any>(data)
  ...
  private async checkTwilioServiceCredentials(
    ...
    const CreadentialCheck = await this.messagingAPIService
      .ValidateCredentials<ITwilioCredentials, boolean>(null, credentials)
      .toPromise();
    return Promise.resolve(CreadentialCheck);
  }
  ...
  private SetCompanyName(result: any) {
    if (this.authService.CompanyId === result.companyId) {
  ...
  async UpdateTwilioConfigurations() {
    ...
    if (twilioAccountSID && twilioAuthKey && twilioServiceId) {
      ...
      if (!IsAccountLinked) {
        this.AppNotificationService.NotifyError('SMS configuration credentials is invalid.');
  ...
  RemoveCompanyIdFromSession() {
    this.browserStorageService.RemoveCompanyId();
  }
