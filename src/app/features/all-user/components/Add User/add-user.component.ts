...
export class AddUserComponent extends AbstractComponent {
  ...
  @Input() AgentDropDownData:IAgentDropdown[];
  @Input() TemplateList: IAgentView[];
  @Input() UserForm: FormGroup;
  @Input() TagList: string[];
  @Input() Roles: IRole[];
  @Input() IsCustomerUserType: boolean;
  @Input() HideIsOverride: boolean;
  ...
  @Output() OnChangeCompany: EventEmitter<string> = new EventEmitter();
  ...
  @Output() SearchBranchNamesOrTags = new EventEmitter();
  ...
  public BindRoleByCompany(event: ICompanyDropDown) {
    this.OnChangeCompany.emit(event.companyId);
  }
  ...
  public ResetControlsOnType(){
    this.OnChangeType.emit();
  }

  public DisableBranches(){
    this.isAllBranchesStatus.emit();
  }
  ...
  public FilterBranch(searchText: string){
    this.SearchBranchNamesOrTags.emit(searchText);
  }
